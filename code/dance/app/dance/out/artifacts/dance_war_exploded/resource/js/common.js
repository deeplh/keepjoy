		...
		$.ajax({
			...
			success : function(data){
				...
				if(data.exceptionCode == "89"){
					window.location.href = $("base").attr("href") + "admin/error/access_denied.jsp";
					return;
				}				
				// cog replaced here...
				container.css({
		...
		$.ajax({
			...
			error : function(xhr, ajaxOptions, thrownError){
				...
				if(xhr.status == 404){
					...
					//window.location.href="admin/modules/error/404.jsp";
					return;
...
//					window.location.href="admin/modules/error/500.jsp";
					return;
				}				
			}
		});
